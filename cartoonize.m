...
imedge = edge(imgry,'Canny'); % Use canny edge detection for detection of edges
...
imedge1 = bwareaopen(imedge,100); % Specify the maximum area of regions you want to remoce
imedge1 = bwareaopen(~imedge1,100);
...
newimageb = zeros(size(imgray));
% Obtain color components of the original image
imr = imrgb(:,:,1);
img = imrgb(:,:,2);
imb = imrgb(:,:,3);
% Color each of the regions with mean color obtained from RGB image 
for i = 1:noOfRegions
...
% Now construction the newimage
newimage(:,:,1) = newimager ;
newimage(:,:,2) = newimageb ;
newimage(:,:,3) = newimageg ;
imshow(newimage); % Show the final reconstructed image
